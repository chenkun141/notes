...
> 读者需要注意的是,CAP理论中的CA和数据库事务中ACID的CA并不完全是同一回事儿.两者之中的C都是一致性(Consistency).CAP中的A是可用性(Availability),而ACID中的A指的是原子性(Atomicity).切勿混为一谈.

### CAP的定义 ###
#### 一致性(Consistency)  ####
一致性指"**all nodes see the same data at the same time**",即更新操作成功并返回客户端完成后,所有节点在同一时间的数据完全一致.所以,一致性,说的就是数据一致性.  

对于一致性,可以分为从客户端和服务端两个不同的时间.从客户端来看,一致性主要指的是多并发访问时更新过的数据如何获取的问题.从服务端来看,则是更新如何复制分布到整个系统,以保证数据最终一致.  

一致性是应为有并发读写才有的问题,因此在理解一致性的问题时,一定要注意结合考虑并发读写的场景.  
